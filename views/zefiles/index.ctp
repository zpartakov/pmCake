<?php
$this->pageTitle="Documents";
?>
<div class="zefiles index">
	<h2><?php echo $this->pageTitle;?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id');?></th>
			<th><?php echo $this->Paginator->sort('task_id');?></th>
			<th><?php echo $this->Paginator->sort('name');?></th>
			<th><?php echo $this->Paginator->sort('type');?></th>
			<th><?php echo $this->Paginator->sort('size');?></th>
			<th><?php echo $this->Paginator->sort('created');?></th>
			<th><?php echo $this->Paginator->sort('modified');?></th>
			<th class="actions"><?php __('Actions');?></th>
	</tr>
	<?php
	$i = 0;
	foreach ($zefiles as $zefile):
		$class = null;
		if ($i++ % 2 == 0) {
			$class = ' class="altrow"';
		}
	?>
	<tr<?php echo $class;?>>
		<td><?php echo $zefile['Zefile']['id']; ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($zefile['Zefile']['task_id'], array('controller'=>'pm_tasks','action' => 'edit', $zefile['Zefile']['task_id'])); ?>
		
		</td>
		<td><?php echo "<a href=\"../files/" .$zefile['Zefile']['task_id']."/" .$zefile['Zefile']['name'] ."\">" .$zefile['Zefile']['name'] ."</a>"; ?>&nbsp;</td>
		<td><?php echo $zefile['Zefile']['type']; ?>&nbsp;</td>
		<td><?php echo $zefile['Zefile']['size']; ?>&nbsp;</td>
		<td><?php echo $zefile['Zefile']['created']; ?>&nbsp;</td>
		<td><?php echo $zefile['Zefile']['modified']; ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View', true), array('action' => 'view', $zefile['Zefile']['id'])); ?>
			<?php echo $this->Html->link(__('Edit', true), array('action' => 'edit', $zefile['Zefile']['id'])); ?>
			<?php echo $this->Html->link(__('Delete', true), array('action' => 'delete', $zefile['Zefile']['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $zefile['Zefile']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page %page% of %pages%, showing %current% records out of %count% total, starting on record %start%, ending on %end%', true)
	));
	?>	</p>

	<div class="paging">
		<?php echo $this->Paginator->prev('<< ' . __('previous', true), array(), null, array('class'=>'disabled'));?>
	 | 	<?php echo $this->Paginator->numbers();?>
 |
		<?php echo $this->Paginator->next(__('next', true) . ' >>', array(), null, array('class' => 'disabled'));?>
	</div>
</div>
<div class="actions">
	<h3><?php __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Zefile', true), array('action' => 'add')); ?></li>
	</ul>
</div>
